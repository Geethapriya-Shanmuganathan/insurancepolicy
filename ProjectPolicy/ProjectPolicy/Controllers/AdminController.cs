...
namespace InsurancePolicyMS.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public AdminController(InsuranceDbContext db) { _db = db; }
        ...
        // User policies mgmt
        [HttpGet("user/{userId}/policies")] public async Task<IActionResult> GetUserPolicies(int userId) { var data = await _db.UserPolicies.Include(up => up.Policy).Where(up => up.UserId == userId).ToListAsync(); return Ok(data); }
        [HttpPost("user/{userId}/policies/{policyId}")] public async Task<IActionResult> AssignPolicy(int userId, int policyId) { if (!await _db.Users.AnyAsync(u => u.UserId == userId) || !await _db.Policies.AnyAsync(p => p.PolicyId == policyId)) return NotFound(); if (await _db.UserPolicies.AnyAsync(up => up.UserId == userId && up.PolicyId == policyId)) return Conflict("Already assigned"); _db.UserPolicies.Add(new UserPolicy { UserId = userId, PolicyId = policyId, PurchaseDate = DateTime.UtcNow }); await _db.SaveChangesAsync(); return Ok(); }
        [HttpDelete("user/{userId}/policies/{policyId}")] public async Task<IActionResult> RemoveUserPolicy(int userId, int policyId) { var up = await _db.UserPolicies.FirstOrDefaultAsync(x => x.UserId == userId && x.PolicyId == policyId); if (up == null) return NotFound(); _db.UserPolicies.Remove(up); await _db.SaveChangesAsync(); return NoContent(); }
